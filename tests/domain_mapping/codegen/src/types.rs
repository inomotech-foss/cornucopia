...
#[derive(Debug)]
pub struct SimRefBorrowed {
    pub id: i32,
    pub iccid: iccid_type::Iccid,
}
impl From<SimRefBorrowed> for SimRef {
    fn from(SimRefBorrowed { id, iccid }: SimRefBorrowed) -> Self {
        Self {
            id,
            iccid: iccid.into(),
        }
...
impl<'a> postgres_types::FromSql<'a> for SimRefBorrowed {
    fn from_sql(
        ty: &postgres_types::Type,
        out: &'a [u8],
    ) -> Result<SimRefBorrowed, Box<dyn std::error::Error + Sync + Send>> {
        let fields = match *ty.kind() {
            postgres_types::Kind::Composite(ref fields) => fields,
            _ => unreachable!(),
        };
        ...
        let _oid = postgres_types::private::read_be_i32(&mut out)?;
        let iccid = postgres_types::private::read_value(fields[1].type_(), &mut out)?;
        Ok(SimRefBorrowed { id, iccid })
    }
    fn accepts(ty: &postgres_types::Type) -> bool {
        ty.name() == "sim_ref" && ty.schema() == "public"
    }
}
impl<'a> postgres_types::ToSql for SimRefBorrowed {
    fn to_sql(
        &self,
        ty: &postgres_types::Type,
        out: &mut postgres_types::private::BytesMut,
    ) -> Result<postgres_types::IsNull, Box<dyn std::error::Error + Sync + Send>> {
        let SimRefBorrowed { id, iccid } = self;
        let fields = match *ty.kind() {
            postgres_types::Kind::Composite(ref fields) => fields,
            _ => unreachable!(),
        };
